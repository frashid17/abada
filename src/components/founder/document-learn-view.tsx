...
export function DocumentLearnView({ payload, layout = "card" }: DocumentLearnViewProps) {
  ...
  }, [isFullscreen, payload.clauses]);

  useEffect(() => {
    const root = isFullscreen ? null : documentRef.current;
    if (!isFullscreen && !root) return;

    const articles = (root ?? documentRef.current)?.querySelectorAll<HTMLElement>(
      "[data-clause-id]",
    );
    ...
    const observer = new IntersectionObserver(
      ...
      },
      {
        ...
        rootMargin: isFullscreen ? "-20% 0px -55% 0px" : "-12% 0px -48% 0px",
      },
    );
  ...
  const clausesList = (
    <div
      className={cn(
        ...
        isFullscreen
          ? "max-w-3xl"
          : "surface-elevated card-shine max-w-2xl space-y-4 rounded-2xl border border-border/60 p-5 shadow-soft sm:p-6",
      )}
    >
      {payload.clauses.map((clause) => (
        <div key={clause.id} className="flex gap-3 sm:gap-4">
          ...
          <article
            ...
            className={cn(
              "min-w-0 flex-1 cursor-pointer scroll-mt-24 rounded-xl border px-4 py-4 text-left transition-all duration-200",
  ...
  if (isFullscreen) {
    return (
      <section className="bg-background pb-16">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_min(420px,36%)]">
          <div
            ref={documentRef}
          ...
          <aside className="bg-muted/5 lg:sticky lg:top-[6.75rem] lg:self-start lg:max-h-[calc(100dvh-7.5rem)] lg:overflow-y-auto">
            ...
            <p className="border-t border-border/50 px-4 py-3 text-[10px] leading-relaxed text-muted-foreground sm:px-6">
              {t("prototypeNote")}
            </p>
          </aside>
        </div>
    ...
    <section className="relative overflow-hidden rounded-2xl border border-border/60 bg-card shadow-card">
      ...
      <div className="relative grid lg:grid-cols-[minmax(0,1fr)_min(380px,34%)]">
        <div className="flex min-h-0 flex-col border-b border-border/50 lg:border-b-0 lg:border-r">
          ...
          <div
            ref={documentRef}
            className="max-h-[min(68vh,680px)] min-h-[420px] overflow-y-auto px-4 py-5 sm:px-6 sm:py-6"
          >
            {clausesList}
          </div>
        </div>
